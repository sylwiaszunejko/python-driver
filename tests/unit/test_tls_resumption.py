...
from tests.tls_certificates import HAVE_CRYPTOGRAPHY, write_self_signed_cert



def _wait_for_the_monotonic_clock_to_move():
    """
    Block until ``time.monotonic()`` reports a new value.

    The deadline a store computes works out to the mark taken when the
    handshake began plus the lifetime the peer announced -- the "now" in
    ``lifetime - (now - mark)`` and the one in ``now + lifetime`` cancel out --
    so two handshakes the clock cannot tell apart are given the same deadline.
    That is the right answer, there being no difference to record, but it
    leaves nothing for a test comparing two deadlines to see.  Windows resolves
    this clock to about ten milliseconds, which two loopback handshakes fit
    inside comfortably.
    """
...
@unittest.skipIf(not HAVE_CRYPTOGRAPHY,
                 'cryptography is required to generate a test certificate')
class TlsResumptionTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._cert_path, cls._key_path = write_self_signed_cert(cls._cert_dir.name)
        ...
        cls._untrusted_cert, cls._untrusted_key = write_self_signed_cert(
    ...
    def test_a_cached_session_pins_its_context_until_the_cache_drops_it(self):
        ...
        # reachable from it -- alive.  A cache the driver created for a cluster
        # is dropped with it; one the caller supplied outlives it, holding the
        # contexts of at most max_size peers until clear() drops them.
        ...
        gc.collect()
        # The entry still holds it, which is the retention the cache documents.
    ...
    def test_a_connection_that_resumed_remembers_what_it_offered(self):
        # Nothing is left to retract once the handshake stands, but the store
        # reads what was offered: handing it to the cache is what tells a
        # session the peer reissued from the one that came back unchanged.
        context = self.make_ssl_context()
        first = self.connect(context)
        first._store_tls_session()
        offered = self.cache.get(first._tls_session_cache_key())

        resumed = self.connect(context)

        assert resumed.session_reused
        assert resumed._tls_session_offered.id == offered.id
    ...
    def test_what_a_resumed_handshake_stores_back(self):
        # SSLSocket.session builds a new object on every access, so comparing
        # object identity here would pass whatever happened.  What matters is
        # whether the peer issued a new session: below TLS 1.3 an abbreviated
        # handshake hands back the one that was offered, and its deadline has
        # to stay where it was rather than start again on every reuse.
        context = self.make_ssl_context()
        first = self.connect(context)
        first._store_tls_session()
        ...
        first_id = self.cache.get(key).id
        first_deadline = self.cache._sessions[key][1]

        # Otherwise both handshakes may begin within one tick of the clock, and
        # a deadline computed afresh is indistinguishable from an inherited one.
        _wait_for_the_monotonic_clock_to_move()
        resumed = self.connect(context)
        assert resumed.session_reused
        resumed._store_tls_session()

        renewed = self.tls_version >= ssl.TLSVersion.TLSv1_3
        assert (self.cache.get(key).id != first_id) is renewed
        assert (self.cache._sessions[key][1] > first_deadline) is renewed


@unittest.skipUnless(ssl.HAS_TLSv1_3, 'this build of OpenSSL has no TLS 1.3')
class Tls13ResumptionTest(TlsResumptionTest):
    """
    The same coverage over TLS 1.3, plus what is specific to it.

    Every inherited test drives a TLS 1.3 server, so a build without it has to
    skip the class rather than fail each handshake.  The guard sits here and not
    on the base class: skipping a base skips its subclasses too, which would
    take these tests out on a build that has TLS 1.3 but not 1.2.
    """
