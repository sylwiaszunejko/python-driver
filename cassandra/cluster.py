...
class Cluster(object):
    ...
    * EXPERIMENTAL: :class:`cassandra.io.asyncioreactor.AsyncioConnection`
    ...
    def ssl_session_cache(self):
        ...
        in its ``client_encryption_options``.  That defaults to true from Scylla
        2026.3, and to false in the releases before it, so a cluster on an
        earlier one has to be told to issue them.  Without them nothing resumes
        and every connection performs a full handshake, as it would have anyway:
        over TLS 1.3 the cache stays empty, while over TLS 1.2 such a server
        still assigns a session id, so the cache may hold an entry it will not
        honour -- offering that costs nothing and the handshake simply completes
        in full."""
