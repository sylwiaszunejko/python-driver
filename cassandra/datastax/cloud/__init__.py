...
def get_cloud_config(cloud_config):
    ...
    try:
        ...
        raise ValueError("Unable to open the zip file for the cloud config. Check your secure connect bundle.") from err

    config = read_metadata_info(config, cloud_config)
    return config


def _safe_extractall(zipfile, tmp_dir):
    for member in zipfile.namelist():
        target = os.path.realpath(os.path.join(tmp_dir, member))
        if not target.startswith(os.path.realpath(tmp_dir) + os.sep):
            raise ValueError("Secure connect bundle contains an unsafe path: %s" % member)
...
def read_cloud_config_from_zip(cloud_config):
    ...
    with ZipFile(secure_bundle) as zipfile:
        ...
        try:
            _safe_extractall(zipfile, tmp_dir)
