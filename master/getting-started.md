...
First, make sure you have the driver properly [installed](https://python-driver.docs.scylladb.com/master/installation.md).

## Connecting to a Cluster

Before we can start executing any queries against a Scylla cluster we need to setup
an instance of [`Cluster`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster). As the name suggests, you will typically have one
instance of [`Cluster`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster) for each Scylla cluster you want to interact
with.

First, make sure you have the Scylla driver properly [installed](https://python-driver.docs.scylladb.com/master/installation.md).

### Connecting to Scylla

The simplest way to create a [`Cluster`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster) is like this:
...
The set of IP addresses we pass to the [`Cluster`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster) is simply
...
Instantiating a [`Cluster`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster) does not actually connect us to any nodes.
To establish connections and begin executing queries we need a
[`Session`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session), which is created by calling [`Cluster.connect()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster.connect):
...
The [`connect()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Cluster.connect) method takes an optional `keyspace` argument
which sets the default keyspace for all queries made through that [`Session`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session):
...
You can always change a Session’s keyspace using [`set_keyspace()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.set_keyspace) or
...
Now that we have a [`Session`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session) we can begin to execute queries. The simplest
way to execute a query is to use [`execute()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.execute):
...
can change the [`row_factory`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.row_factory) attribute.
...
To prepare a query, use [`Session.prepare()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.prepare):
...
[`prepare()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.prepare) returns a [`PreparedStatement`](https://python-driver.docs.scylladb.com/master/api/cassandra/query.md#cassandra.query.PreparedStatement) instance
which can be used in place of [`SimpleStatement`](https://python-driver.docs.scylladb.com/master/api/cassandra/query.md#cassandra.query.SimpleStatement) instances or literal
...
[`execute_async()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.Session.execute_async).  Instead of waiting for the query to
complete and returning rows directly, this method almost immediately
returns a [`ResponseFuture`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture) object.  There are two ways of
getting the final result from this object.

The first is by calling [`result()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture.result) on it. If
...
Alternatively, instead of calling [`result()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture.result),
you can attach callback and errback functions through the
[`add_callback()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture.add_callback),
[`add_errback()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture.add_errback), and
[`add_callbacks()`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ResponseFuture.add_callbacks), methods.  If you have used
...
By default, [`ConsistencyLevel.LOCAL_ONE`](https://python-driver.docs.scylladb.com/master/api/cassandra.md#cassandra.ConsistencyLevel.LOCAL_ONE) will be used for all queries.
You can specify a different default by setting the [`ExecutionProfile.consistency_level`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.ExecutionProfile.consistency_level)
for the execution profile with key [`EXEC_PROFILE_DEFAULT`](https://python-driver.docs.scylladb.com/master/api/cassandra/cluster.md#cassandra.cluster.EXEC_PROFILE_DEFAULT).
To specify a different consistency level per request, wrap queries
in a [`SimpleStatement`](https://python-driver.docs.scylladb.com/master/api/cassandra/query.md#cassandra.query.SimpleStatement):
...
The second option is to create a [`BoundStatement`](https://python-driver.docs.scylladb.com/master/api/cassandra/query.md#cassandra.query.BoundStatement) from the
[`PreparedStatement`](https://python-driver.docs.scylladb.com/master/api/cassandra/query.md#cassandra.query.PreparedStatement) and binding parameters and set a consistency
level on that:
...
* Configure a [`SpeculativeExecutionPolicy`](https://python-driver.docs.scylladb.com/master/api/cassandra/policies.md#cassandra.policies.SpeculativeExecutionPolicy) with the ExecutionProfile
