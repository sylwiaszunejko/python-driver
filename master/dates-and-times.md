...
ranges without overflow, this driver returns these data in custom types: [`util.Date`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Date) and [`util.Time`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Time).

### Write Path

For simple (not prepared) statements, the input values for each of these can be either a string literal or an encoded
integer. See [Working with dates](https://github.com/apache/cassandra/blob/trunk/doc/cql3/CQL.textile#working-with-dates)
or [Working with time](https://github.com/apache/cassandra/blob/trunk/doc/cql3/CQL.textile#working-with-time) for details
on the encoding or string formats.

For prepared statements, the driver accepts anything that can be used to construct the [`util.Date`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Date) or
[`util.Time`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Time) classes. See the linked API docs for details.

### Read Path

The driver always returns custom types for `date` and `time`.

The driver returns [`util.Date`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Date) for `date` in order to accommodate the wider range of values without overflow.
For applications working within the supported range of [`datetime.MINYEAR`, `datetime.MAXYEAR`], these are easily
converted to standard `datetime.date` insances using [`Date.date()`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Date.date).

The driver returns [`util.Time`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Time) for `time` in order to retain nanosecond precision stored in the database.
For applications not concerned with this level of precision, these are easily converted to standard `datetime.time`
insances using [`Time.time()`](https://python-driver.docs.scylladb.com/master/api/cassandra/util.md#cassandra.util.Time.time).
